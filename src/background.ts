...
import { errorMessage, meetingFolder, normalizeSettings, safeDownloadPath, validSelection, validSignal, validTileMeta } from "./shared";
...
async function recorder<T>(type: string, payload: object = {}): Promise<T> {
  ...
  if (!reply?.ok) throw new Error(reply && !reply.ok ? reply.error : "The recorder did not respond. Reload the extension and Meet tab.");
  return reply.data;
}
...
async function meetTab() {
  ...
  if (!tab?.id || !tab.url?.startsWith("https://meet.google.com/")) throw new Error("Open your Google Meet tab and use GMRec there.");
...
async function handle(message: any, sender: chrome.runtime.MessageSender): Promise<unknown> {
  ...
  if (sender.tab && !sender.url?.startsWith(chrome.runtime.getURL(""))) {
    if (sender.frameId !== 0 || !sender.url?.startsWith("https://meet.google.com/")) throw new Error("Unsupported recording source.");
...
// A fresh install lands on the walkthrough: GMRec needs consent, device setup and a Meet tab
